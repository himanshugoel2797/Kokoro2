...
namespace Kokoro3.OpenGL
{
    static class LLDevice
    {
        ...
        {
            if (slot >= GraphicsDevice.SystemLimits[HardwareLimits.MaxTextureUnits]) throw new ArgumentException($"The slot #{slot} is invalid, valid values are from 0 to {GraphicsDevice.SystemLimits[HardwareLimits.MaxTextureUnits] - 1}");
            int prev = curSlot;
            curSlot = slot;
