...
namespace Kokoro3.OpenGL
{
    ...
    static class ImageDDS
    {
        ...
        public static void LoadFromDisk(string filename, out TextureTarget dimension, out int width, out int height, out PixelInternalFormat pct)
        {
            #region Prep data
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file. 
            {
                ...
                if (CheckFlag(pfFlags, (uint)eDDPF.FOURCC))
                    ...
                    {
                        case eFOURCC.DXT1:
                            _PixelInternalFormat = (PixelInternalFormat)All.CompressedRgbS3tcDxt1Ext;
                            _BytesPerBlock = 8;
                            _IsCompressed = true;
                            break;
                        //case eFOURCC.DXT2:
                        case eFOURCC.DXT3:
                            _PixelInternalFormat = (PixelInternalFormat)All.CompressedRgbaS3tcDxt3Ext;
                            _BytesPerBlock = 16;
                            _IsCompressed = true;
                            break;
                        //case eFOURCC.DXT4:
                        case eFOURCC.DXT5:
                            _PixelInternalFormat = (PixelInternalFormat)All.CompressedRgbaS3tcDxt5Ext;
                            _BytesPerBlock = 16;
                            _IsCompressed = true;
                            break;
                        default:
                ...
                for (int Slices = 0; Slices < _Depth; Slices++)
                {
                    ...
                    for (int Level = 0; Level < _MipMapCount; Level++) // start at base image
                    {
                        ...
                        { // Note: there could be a potential problem with non-power-of-two cube maps
                            ...
                            {  // Turn the blocks upside down and the rows aswell, done in a single pass through all blocks
                                for (int sourceColumn = 0; sourceColumn < BlocksPerColumn; sourceColumn++)
                                {
                                    ...
                                    for (int row = 0; row < BlocksPerRow; row++)
                                    {
                                        ...
                                        {
                                            case (PixelInternalFormat)All.CompressedRgbS3tcDxt1Ext:
                                                // Color only
                                                ...
                                                break;
                                            case (PixelInternalFormat)All.CompressedRgbaS3tcDxt3Ext:
                                                // Alpha
                                                ...
                                                break;
                                            case (PixelInternalFormat)All.CompressedRgbaS3tcDxt5Ext:
                            ...
                            pct = (PixelInternalFormat)internalformat;
                            GLError = GL.GetError();
