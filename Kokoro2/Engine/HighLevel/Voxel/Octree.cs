    ...
    new int[4]{3,2,1,0},new int[4]{7,5,6,4},new int[4]{11,10,9,8}
        ...
        public static Vector3 ApproximateZeroCrossingPosition(Vector3 p0, Vector3 p1)
        {
            ...
            while (currentT <= 1.0f)
            {
                ...
                float density = System.Math.Abs(glm.Density_Func(p));
                if (density < minValue)
                {
                    minValue = density;
                    t = currentT;
        ...
        public static OctreeNode BuildOctree(Vector3 min, int size, float threshold)
        {
            Kokoro2.Debug.ErrorLogger.AddMessage(-1, $"Building Octree at {min}, with size of {size} and threshold of {threshold}", Kokoro2.Debug.DebugType.Marker, Kokoro2.Debug.Severity.Notification);
        ...
        public static void GenerateMeshFromOctree(OctreeNode node, List<MeshVertex> vertexBuffer, List<int> indexBuffer)
        {
            ...
            float[] norms = new float[vertexBuffer.Count * 3];
            float[] verts = new float[vertexBuffer.Count * 3];
            float[] uvs = new float[vertexBuffer.Count * 2];

            for (int i = 0; i < vertexBuffer.Count; i++)
            {
                verts[3 * i] = vertexBuffer[i].xyz.X;
                verts[3 * i + 1] = vertexBuffer[i].xyz.Y;
                verts[3 * i + 2] = vertexBuffer[i].xyz.Z;

                norms[3 * i] = vertexBuffer[i].normal.X;
                norms[3 * i + 1] = vertexBuffer[i].normal.Y;
                norms[3 * i + 2] = vertexBuffer[i].normal.Z;

                uvs[2 * i] = vertexBuffer[i].xyz.X;
                uvs[2 * i + 1] = vertexBuffer[i].xyz.Z;
            }

            /*TODO GameObject go = new GameObject("Mesh");
            Mesh mesh;
            MeshFilter filter;
