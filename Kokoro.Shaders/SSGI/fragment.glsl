...
poissonDisk[63] = vec2(-0.178564, -0.596057);
	...
	vec4 lDir = (Projection * View *  vec4(giLightDirection, 0));

	vec2 lCoord = lDir.xy * 0.5 + 0.5;
	/*
	float ao = depth;
	for(int i = 0; i < 64; i++)
	{
		float d_a = texture2D(depthMap, UV + lCoord * 0.025 + poissonDisk[i] * 0.025).r;
		vec3 n_a = decode(texture2D(normData, UV + lCoord * 0.025 + poissonDisk[i] * 0.025).xy);
		ao += (depth - d_a);// * (1 - dot(n_a, -giLightDirection));
